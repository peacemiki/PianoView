...
public class Piano {
    ...
    private void initPiano() {
        int[] whiteKeyDrawables = {R.drawable.white_piano_key1, R.drawable.white_piano_key2, R.drawable.white_piano_key3, R.drawable.white_piano_key4, R.drawable.white_piano_key5};

        if (scale > 0) {
            //获取黑键和白键的高度和宽度
            Drawable blackDrawable = ContextCompat.getDrawable(context, R.drawable.black_piano_key);
            Drawable whiteDrawable = ContextCompat.getDrawable(context, R.drawable.white_piano_key1);
            ...
            for (int i = 0; i < WHITE_PIANO_KEY_GROUPS; i++) {
                ...
                for (int j = 0; j < mKeys.length; j++) {
                    ...
                    mKeys[j].setKeyDrawable(
                            new ScaleDrawable(ContextCompat.getDrawable(context, whiteKeyDrawables[j % whiteKeyDrawables.length]),
                                    Gravity.NO_GRAVITY, 1, scale).getDrawable());
